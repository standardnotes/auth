...
export class MySQLRevokedSessionRepository
  extends Repository<RevokedSession>
  implements RevokedSessionRepositoryInterface
{
  ...
  async findOneByUuid(uuid: string): Promise<RevokedSession | undefined> {
    return this.createQueryBuilder('revoked_session').where('revoked_session.uuid = :uuid', { uuid }).getOne()
  }
}
