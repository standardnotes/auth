...
describe('MySQLSessionRepository', () => {
  ...
  it('should find one session by id', async () => {
    ...
    expect(queryBuilder.where).toHaveBeenCalledWith('s.uuid = :uuid', { uuid: '123' })
    expect(result).toEqual(session)
  })
  ...
  it('should find one session by id and user id', async () => {
    ...
    expect(queryBuilder.where).toHaveBeenCalledWith('s.uuid = :uuid AND s.user_uuid = :user_uuid', { uuid: '123', user_uuid: '234' })
    expect(result).toEqual(session)
  })
